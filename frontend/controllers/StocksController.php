...
class StocksController extends CommonController{
    ...
    public function actionIndex()
    {
        ...
        $query = (new \yii\db\Query())->from($tablePrefix.'stocks as s')
            ...
            ->leftJoin(['w'=>$tablePrefix.'warehouse'],'w.warehouse_id=s.warehouse_id')
            ->where($where)->orderBy(['s.goods_id'=>SORT_DESC])->groupBy(['s.warehouse_id','s.goods_id']);
        if (!empty($s_con['goods_name'])) $query->andFilterWhere(['like', 's.goods_name',$s_con['goods_name']]); //商品名称
        if (!empty($s_con['barode_code'])) $query->andFilterWhere(['like', 's.barode_code',$s_con['barode_code']]); //条形码
        if (!empty($s_con['brand_name'])) $query->andFilterWhere(['like', 's.brand_name',$s_con['brand_name']]); //品牌
        $countQuery = clone $query;
        $pages = new Pagination(['totalCount' => $countQuery->count(),'defaultPageSize'=>22]);
