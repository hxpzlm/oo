...
class CstocksController extends CommonController{
    ...
    public function actionIndex(){
        ...
        $buy_time_start = empty(Yii::$app->request->get('buy_time_start'))? 0:strtotime(Yii::$app->request->get('buy_time_start')); //采购入库开始时间
        $buy_time_end = empty(Yii::$app->request->get('buy_time_start'))? time():strtotime(Yii::$app->request->get('buy_time_end')); //采购退货入库结束时间
        $query = (new \yii\db\Query())->from($tablePrefix.'purchase as p')
            ...
            ->leftJoin(['g'=>$tablePrefix.'purchase_goods'],'g.purchase_id=p.purchase_id')
            ->leftJoin(['w'=>$tablePrefix.'warehouse'],'w.warehouse_id=p.warehouse_id')
            ->where($where)->orderBy(['p.purchases_status'=>SORT_ASC,'p.buy_time'=>SORT_DESC]);
        if (!empty($s_con['goods_name'])) $query->andFilterWhere(['like', 'g.goods_name',$s_con['goods_name']]); //商品名称
        if (!empty($s_con['barode_code'])) $query->andFilterWhere(['like', 'g.barode_code',$s_con['barode_code']]); //条形码
        if (!empty($s_con['brand_name'])) $query->andFilterWhere(['like', 'g.brand_name',$s_con['brand_name']]); //品牌
        ...
        if($action=="comfirm"){
            ...
            try{
                ...
                foreach($goods as $v){
                    ...
                    if(!$model->save()){
                        throw new \Exception($v['goods_name']."商品入库失败！");
                    }
                }
