...
<div class="seeks clearfix">
    ...
    <p class="seeks-xl">更多搜索条件<label>▼</label><?php if(strtotime(Yii::$app->request->get('refuse_time_start'))>strtotime(Yii::$app->request->get('refuse_time_end'))){?>&nbsp;&nbsp;<strong style="color: red; margin: 0; background: none;">开始时间不能大于结束时间</strong><?php };?></p>
...
<div class="seeks-box clearfix">
    ...
    <div class="seeks-boxs clearfix">
        <p>入库状态</p>
        <select name="status">
            <option <?php if(Yii::$app->request->get('status')==''){echo 'selected';};?> value="">请选择</option>
            <option <?php if(Yii::$app->request->get('status')==2){echo 'selected';};?> value="2">未入库</option>
            <option <?php if(Yii::$app->request->get('status')==1){echo 'selected';};?> value="1">已入库</option>
        </select>
    </div>
        ...
        <div class="seeks-boxs clearfix">
            ...
            <select name="store_id">
                ...
                <?php foreach($store_list as $value){?>
                    <option <?php if($value['store_id']==Yii::$app->request->get('purchases_status')){echo 'selected';};?> value="<?=$value['store_id']?>"><?=$value['name']?></option>
                <?php }?>
            </select>
        </div>
    <?php }?>
    <div class="seeks-boxs seeks-boxst2 clearfix">
        ...
        <span>-</span>
        <input type="text" placeholder="退货终止时间" name="refuse_time_end" id='end-date' class="laydate-icon" value="<?=Yii::$app->request->get('refuse_time_end')?Yii::$app->request->get('refuse_time_end'):date('Y-m-d', time())?>"/>
    </div>
</div>
<?php ActiveForm::end(); ?>
